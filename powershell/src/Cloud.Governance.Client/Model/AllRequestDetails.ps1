...
<#
AllRequestDetails<PSCustomObject>
#>

function New-AllRequestDetails {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: Cloud.Governance.Client => AllRequestDetails' | Write-Debug
...
AllRequestDetails<PSCustomObject>
#>
function ConvertFrom-JsonToAllRequestDetails {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "LastApprovedTime"))) { #optional property not found
            $LastApprovedTime = $null
        } else {
            $LastApprovedTime = $JsonParameters.PSobject.Properties["LastApprovedTime"].value
        }

...
function Get-GroupMetadata {
    ...
    Process {
        'Calling method: Get-GroupMetadata' | Write-Debug
        ...
        $LocalVarUri = '/workspace/groups/{id}/metadata'
        if (!$Id) {
            throw "Error! The required parameter `Id` missing when calling getGroupMetadata."
        }
...
function Get-SiteMetadata {
    ...
    Process {
        ...
        if (!$Url) {
            throw "Error! The required parameter `Url` missing when calling getSiteMetadata."
        }
...
.PARAMETER Orderby
Order by one field, supported fields:<br/> id, name, type, siteUrl, groupEmail, primaryContact, phase, isCurrentRenewer, createdTime, status, autoImportProfileId, pendingAction, secondaryContact, policy, policyId, description, primaryAdmin, additionalAdmin, geoLocation, storageLimit, storageUsage, classification, privacy
